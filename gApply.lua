...
local gApply, parent = torch.class('nn.gApply', 'nn.Module')

function gApply:__init()
  parent.__init(self)
...
function gApply:updateOutput(inputs)
  ...
  for o=1,self.numOutputs do
    ...
    self.outputs[o]:resizeAs(inputs[1]) -- will be same size, since gApply is strictly per-element, no Narrows and stuff
  end
  forwardParams = {}
...
function gApply:updateGradInput(inputs, gradOutputs)
  print('gApply:updateGradInput')
  if torch.type(inputs) == 'torch.ClTensor' then
    inputs = {inputs}
  end
